...
| `backend/app.py` | 255 | 105 | **59%** | ⚠️ Missing tests |
| `backend/helpers/data_loader.py` | 17 | 0 | **100%** | ✅ Excellent |
| `backend/helpers/acl_parser.py` | 871 | 451 | **48%** | 🚨 Missing |

| `backend/helpers/version_checker.py` | 53 | 53 | **0%** | 🚨 NO |
...
Selectors are a critical Redis 7.0+ feature that allows multiple ACL rules in a
...
**Why this matters:**
CLAUDE.md claims selector support was added in v1.20.0, but we have NO tests
...
The `version_checker.py` module has 53 statements and **ZERO test coverage**.
...
- test_advanced_features.py: 38 tests (✅ **+30 tests** - module coverage, OSS
features)
...
While we have **fewer total tests**, we added significant **Redis 8 module
coverage** and **OSS-specific tests** (cluster, replication, latency). However,
we lost **critical selector tests** and **API endpoint tests**.
...
1. Restore advanced parser tests (5 tests)
1. Restore core ACL parser tests (3 tests)
1. Add missing edge case coverage based on HTML coverage report

**Estimated impact:** +8 tests, ~10% coverage increase

### Phase 3: Nice-to-Have (Target: 90% coverage)

1. UI/Integration tests (3 tests)
1. Additional edge cases from coverage report analysis
...
- Priority should be selector tests - this is a major claimed feature with 0%
  test coverage!
- version_checker.py needs tests created from scratch (no historical tests
  exist)
