...
docker run -d -p 7380:7380 --name redis-acl-builder redis-acl-builder:v1.20.0-beta
```
...
docker buildx build \
  ...
  --tag YOUR_USERNAME/redis-acl-builder:v1.20.0-beta \
  --push \
  -f Dockerfile ..

# Pull and run on any supported architecture
docker run -d -p 7380:7380 YOUR_USERNAME/redis-acl-builder:v1.20.0-beta
```
...
docker buildx imagetools inspect redis-acl-builder:v1.20.0-beta-multiarch
```
